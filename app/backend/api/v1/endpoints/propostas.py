...
@router.post("/{proposta_id}/rebuild", response_model=PropostaRebuildResponse)
async def rebuild_proposta(
    proposta_id: UUID,
    current_user=Depends(get_current_active_user),
    db: AsyncSession = Depends(get_db),
) -> PropostaRebuildResponse:
    """Rebuild proposal totals after histogram edits or extra resource changes."""
    await require_proposta_role(proposta_id, PropostaPapel.EDITOR, current_user, db)
    svc = PropostaMontagemService(db)
    resultado = await svc.rebuild(proposta_id)
    await db.commit()
    return PropostaRebuildResponse(**resultado)
