...
def test_normalize_collapses_whitespace():
    assert normalize_text("concreto   fck  25") == "concreto fck 25"


def test_normalize_removes_accents():
    assert normalize_text("Ação de Fundação") == "acao de fundacao"
...
async def test_fase1_returns_result_and_assoc_when_association_found():
    ...
    from app.models.enums import StatusHomologacao, StatusValidacaoAssociacao
    ...
    mock_servico.custo_unitario = Decimal("45.50")
...
async def test_fase1_returns_none_when_servico_inactive():
    ...
    servico_repo = AsyncMock()
    servico_repo.get_active_by_id = AsyncMock(return_value=None)  # soft deleted
...
async def test_fase3_uses_batch_load_not_n_plus_1():
    ...
    servico_repo = AsyncMock()
    servico_repo.get_active_by_ids = AsyncMock(return_value=mock_servicos)
    # get_active_by_id must NOT be called — that would be N+1
    servico_repo.get_active_by_id = AsyncMock(side_effect=AssertionError("N+1 detected!"))
    ...
    # Verify batch call was made, N+1 was NOT (get_active_by_id not called)
    servico_repo.get_active_by_ids.assert_called_once_with(ids)
    servico_repo.get_active_by_id.assert_not_called()
