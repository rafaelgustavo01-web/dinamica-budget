...
Após análise da estrutura original do Excel (`openpyxl`), foi descoberto que a PINI utiliza a formatação em **Negrito (`font.bold = True`)** e **Alinhamento à Esquerda (`alignment.indent = 0`)** exclusivamente na célula de Descrição do Serviço Pai da composição. Os subserviços, mesmo começando com `SER.`, não possuem negrito e podem possuir indentação. Como mecanismo de *cross-check*, os itens Pai devem ser validados por essas duas propriedades.

## Critérios de Aceite
1. `app/backend/services/etl_service.py` modificado para usar a detecção de `font.bold` e `alignment.indent == 0` da célula de descrição para definir `current_parent_codigo`.
