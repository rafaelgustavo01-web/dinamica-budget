...
1.  **Fase 0.1: Código Exato:** Circuit-break imediato se o texto de busca for um código de origem idêntico (priorizando itens próprios do cliente).
2.  **Fase 0.2: Itens Próprios:** Busca fuzzy restrita ao catálogo aprovado do cliente.
3.  **Fase 1: Associação Direta:** Lookup na tabela de associações inteligentes (aprendizado do sistema).
4.  **Fase 2/3: Global (Fuzzy + IA):** Fallback para o catálogo global TCPO usando similaridade de texto e vetores (IA Semântica).
...
- **Novos Testes:** Criados em `app/backend/tests/unit/test_f2_07_features.py` cobrindo:
    - Match por código exato no Motor de Busca.
    - Agregação correta de custos por Tipo de Recurso no CPU Service.
    - Aplicação de BDI proporcional no resumo de recursos.
