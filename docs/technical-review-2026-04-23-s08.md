...
- Added smoke E2E for the main proposal flow in `app/tests/e2e/test_smoke_proposta.py`.
...
- The secret scan needed refinement to avoid noisy matches from test code. The final version scans `app` and `frontend/src`, excludes test paths, and keeps the regex set intentionally small and actionable.

## Verification

- `pytest app/tests/e2e/test_smoke_proposta.py -q` -> `1 passed`
- `powershell -ExecutionPolicy Bypass -File scripts\audit-quality-gate.ps1 -ProjectRoot .` -> `0 falhas`
- Quality gate breakdown:
  - `pytest app/tests/unit -q` -> `93 passed`
  - `alembic current` -> database at `head`
  - secret scan -> pass
  - `pytest app/tests/unit/test_security_p0.py app/tests/unit/test_security_s04.py -q` -> `22 passed`
