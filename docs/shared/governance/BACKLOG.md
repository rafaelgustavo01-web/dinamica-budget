...
| `F4-DT-01` | TESTED | P1 | `F4-05` | **QA Hygiene + Backlog/Registry Cleanup** — limpar ruídos conhecidos e manter pipeline confiável antes de retomar M7 | Corrigir handler MSW faltante; corrigir nesting HTML em `ExpandableTreeRow`; decidir mitigação do risco `xlsx`; atualizar `workers.json`; backlog/pipeline sem notas conflitantes |
| `F4-DT-02` | TESTED | P1 | `F4-DT-01` | **QA Residual Debt Cleanup** — corrigir débitos técnicos encontrados no QA das últimas 10 sprints | Status de match PQ derivado do banco; mojibake removido; schemas Pydantic nos endpoints de itens de proposta; artefatos F4 normalizados; gates frontend/backend verdes |
| `F4-DB-01` | BACKLOG | P0 | `F4-05`, `F4-DT-02` | **Alembic/DB Validation Gate** — validar Fase 4 contra banco real/seguro na instância de deploy antes de promover F4 para DONE | Kimi executa backup, alembic current/heads/check/upgrade/downgrade controlado, smoke de tabelas, import/status e relatório com evidências; sem deploy app sem aprovação |
...
- 2026-04-29 (QA/Gemini retry): `F3-01` e `F3-02` aceitas pelo Gemini QA. Technical Feedbacks e walkthroughs reviewed criados; ambas movidas para DONE. `F3-04` aberta para configs/polimento/smoke; `F3-03` fica posterior ao fechamento das configurações finais.
